...
import { downloadAppealText, printAppealDocument, renderMarkdownToHtml } from '../lib/appealExport'
...
export default function AppealLetterViewer({
    ...
    const copyToClipboard = async () => {
        // Copiamos tanto text/html (para que apps como Gmail, Outlook o Word
        // peguen el texto ya formateado: negritas, headers, tablas) como
        // text/plain (fallback para editores que no aceptan HTML).
        try {
            ...
            // Si falla el copiado enriquecido (p. ej. navegador sin soporte de
            // ClipboardItem), reintentamos con texto plano antes de rendirnos.
            try {
                await navigator.clipboard.writeText(currentText)
                setCopied(true)
                setTimeout(() => setCopied(false), 2000)
        ...
        <div className="flex flex-col gap-6" aria-label="Appeal Letter Review">
            <Card variant="elevated">
                ...
                <span className="text-xs font-semibold text-deep flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-mid">
                    Markdown source
                  </span>
                                    <Textarea
                                        value={editedText}
                  ...
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-mid">
                    Live preview
                                    ...
                                    <div className="max-h-[420px] flex-1 overflow-y-auto rounded-[4px] border border-border bg-canvas/30 p-4 font-sans">
                                        {editedText.trim() ? (
                                            <HighlightedMarkdown text={editedText} flags={flags} selectedFlagIndex={null} />
                                        ) : (
                                            <p className="text-xs italic text-mid">Preview will appear here as you type.</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col gap-4 rounded-[4px] border border-border bg-canvas/30 p-6 font-sans">
                            {currentText.trim() ? (
                                <HighlightedMarkdown text={currentText} flags={flags} selectedFlagIndex={selectedFlagIndex} />
                            ) : (
